...
class Cell
{
  ...
  this(AsciiTable table, Row row, string formatted)
  {
    ...
    this.width = lines
      .map!(line => line.unformattedLength)
      .maxElement;
  }
  ulong height()
  {
...
class Row
{
  ...
  auto add(V)(V v)
  {
    ...
    {
      throw new Exception("too many elements in row nrOfColumns=%s cells=%s".format(nrOfColumns, cells.length));
    }
  ...
  auto render(ulong row, string columnSeparator) {
    ...
    return columnSeparator
      ~ cells.map!(cell => cell.render(row)).join(columnSeparator)
      ~ columnSeparator;
  }
  auto width(string columnSeparator) {
    return cells.fold!((memo, cell) => memo + cell.width)(0UL)
      + (cells.length+1) * columnSeparator.length;
  }
  override string toString() {
...
struct Formatter
{
  ...
  string toString()
  {
    ...
    return table
      .rows
      .fold!((memo, row) => toString(memo, table, row, hSeparator, rSeparator))(lines)
      .map!(line => prefix ~ line)
      .join("\n");
  }
    
  private auto toString(string[] lines, AsciiTable table, Row row, string hSeparator, string rSeparator) {
